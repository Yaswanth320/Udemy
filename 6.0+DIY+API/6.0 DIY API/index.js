...
app.get("/jokes/:id", (req,res) =>{
  const specificIndex = parseInt(req.params.id);
  const jokeFound = jokes.find( (joke) => joke.id === specificIndex);
  if(jokeFound){
    res.json(jokeFound);
  }else{
    res.json({
      code : 404,
      message : `no jokes with id: ${specificIndex}`,
    });
  }
...
app.get("/filter", (req, res) => {
  const specificType = req.query.type;
  const filterdJokes = jokes.filter((joke) => joke.jokeType === specificType);
  if(filterdJokes){
    res.json(filterdJokes);
  }else{
    res.json({
      code : 402,
      message : `no jokes found of type ${specificType}`,
    });
  }
});
